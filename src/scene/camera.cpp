...
Matrix4f Camera::projection()
{
    ...
    const float top = std::tan(fov_y / 2.0f) * near_plane;
    ...
    projection(0, 0) = 2.0f * near_plane / (right - left);
    projection(1, 1) = 2.0f * near_plane / (top - bottom);
